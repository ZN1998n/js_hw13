...
for(let items of users){
    let emailEnd = items.email.split('.').at(-1)

    if(emails[emailEnd]){
        emails[emailEnd].push(items.email)
    } else {
        other.push(items.email)
    } 
}
...
for(let i = 0; i < users.length; i++) {
    let emailEnd = users[i].email.split('.').at(-1)
    
    if(emails[emailEnd]){
        emails[emailEnd].push(users[i].email)
    } else {
        other.push(users[i].email)
    }
}
